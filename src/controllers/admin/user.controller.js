...
const addUser = async (req, res) => {
  ...
  const designationQuery =
    "SELECT * FROM designations WHERE designation_id  = ?";
  ...
  try {
    ...
    const insertContrasenaQuery =
      "INSERT INTO contrasena (user_id, extenstions) VALUES (?,?)";
...
const userLogin = async (req, res) => {
  ...
  try {
    ...
    const userDataQuery = `SELECT u.*, c.contrasena_id, d.designation_name FROM  users u 
        ...
        WHERE u.user_id = ? `;
    let userDataResult = await connection.query(userDataQuery, [
      check_user.user_id,
    ]);
...
const updateUser = async (req, res) => {
  ...
  const isDesignationExistsQuery =
    "SELECT * FROM designations WHERE designation_id = ?";
  const isDesignationExistsResult = await pool.query(isDesignationExistsQuery, [
    designation_id,
  ]);
...
//password change
const onPasswordChange = async (req, res) => {
  const email_id = req.body.email_id ? req.body.email_id.trim() : ""; // Extract and trim email_id from request body
  const password = req.body.password ? req.body.password : "";
  ...
  try {
    ...
    const hash = await bcrypt.hash(new_password, 10); // Hash the password using bcrypt
    // Update user password in the database
    const updateQuery = `UPDATE contrasena SET extenstions=? WHERE user_id=?`;
    const updateResult = await connection.query(updateQuery, [
      hash,
      check_user.user_id,
    ]);
...
const checkEmailId = async (req, res) => {
  ...
  try {
    ...
    const check_user = checkUserResult[0][0];
    if (!check_user) {
      return error404(" Email Id not found.", res);
    }
    //commit the transation
    await connection.commit();
    res.status(200).json({
